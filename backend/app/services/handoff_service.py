...
def log_history_event(
    ...
    *,
    ...
    """Append a history event. Caller is responsible for committing."""
...
def get_history(
    db: Session, candidate_id: str, job_id: Optional[str] = None
) -> list[CandidateHistoryEvent]:
    query = db.query(CandidateHistoryEvent).filter(
        CandidateHistoryEvent.candidate_id == str(candidate_id)
    )
    if job_id:
        query = query.filter(CandidateHistoryEvent.job_id == str(job_id))
    return query.order_by(CandidateHistoryEvent.created_at.desc()).all()
...
def create_handoff(
    db: Session, payload: HandoffCreateRequest, recruiter_email: str
) -> InterviewHandoff:
...
def mark_viewed(db: Session, handoff: InterviewHandoff) -> InterviewHandoff:
...
def acknowledge_handoff(
    db: Session, handoff: InterviewHandoff, notes: Optional[str]
) -> InterviewHandoff:
...
def list_handoffs_for_candidate(db: Session, candidate_id: str) -> list[InterviewHandoff]:
    return (
        db.query(InterviewHandoff)
        .filter(InterviewHandoff.candidate_id == str(candidate_id))
        .order_by(InterviewHandoff.created_at.desc())
        .all()
    )
