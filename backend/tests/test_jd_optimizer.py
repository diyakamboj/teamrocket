...
def _add_candidate(db, *, name, email, skills, source="resume_upload"):
    candidate = Candidate(
        id=uuid.uuid4(),
        name=name,
        email=email,
...
def _add_evaluated_candidate(
    ...
    *,
    ...
    candidate = _add_candidate(db, name=name, email=email, skills=skills, source=source)
...
def _platform_job(db) -> JobPosting:
    job = JobPosting(
        id=uuid.uuid4(),
...
def _seed_platform_pool(db, job_id):
    ...
    for index in range(10):
        ...
        _add_evaluated_candidate(
            ...
            pipeline_stage="interviewing" if index == 0 else "screened",
            source="referral" if index == 0 else "resume_upload",
        )
...
def test_coverage_agrees_with_candidate_matcher_normalization(db):
    ...
    candidates = {
        c.id: c
        for c in db.query(Candidate).filter(Candidate.id.in_([e.candidate_id for e in evaluations]))
    }
...
def test_hiring_insights_scopes_to_evaluated_candidates(client, db):
    ...
    assert payload["candidate_sources"]["resume_upload"] == 9
    assert payload["candidate_sources"]["referral"] == 1
...
def test_distribution_is_job_scoped(client, db):
    ...
    assert payload["candidate_sources"]["linkedin"] == 0
    assert payload["pipeline_progression"]["screened"] == 9
