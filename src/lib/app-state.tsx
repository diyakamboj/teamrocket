...
export function AppStateProvider({ children }: { children: ReactNode }) {
  ...
  const counts = useMemo(() => {
    const c = { total: files.length, queued: 0, processing: 0, completed: 0, failed: 0, duplicate: 0 };
  ...
  useEffect(() => {
    if (wasActive.current && !active && counts.total > 0) {
      toast.success(`Batch complete — ${counts.completed} resumes parsed`, {
        description: counts.failed ? `${counts.failed} files failed and can be retried.` : undefined,
        action: {
