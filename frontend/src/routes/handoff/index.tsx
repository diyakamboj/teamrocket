...
function InterviewHandoffPage() {
  const { weights, activeJobId } = useAppState();
  ...
  async function handleCreateHandoff() {
    ...
    try {
      const handoff = await createHandoff({
        ...
        evidence_highlights: selected.evidence.map(
          (e) => `${e.skill}: ${e.detail} (${e.source})`,
        ),
      });
      toast.success(
        `Briefing handed off to ${handoff.interviewer_name}${
          handoff.email_source === "mock" ? " (email logged — SMTP not configured)" : " — email sent"
        }`,
      );
      setInterviewerName("");
      setInterviewerEmail("");
            ...
            <div className="grid gap-6 xl:grid-cols-2">
              <section className="card-surface space-y-4 p-5">
                ...
                <p className="text-xs text-muted-foreground">
                  Centralized log — every re-score, decision, and handoff event for this
              ...
              <section className="card-surface space-y-4 p-5">
                ...
                <p className="text-xs text-muted-foreground">
                  Sends a candidate summary (scorecard, strengths, gaps, suggested focus areas) to
                  a technical interviewer with a trackable link.
                </p>
