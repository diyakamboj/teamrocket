...
> **Status:** Proposal — target design. Not yet implemented.
> **Author:** Senior AI engineer / architect (in support of the TPM/AI engineer)
> **Audience:** the engineering team, and any Claude Code session working in this repo.

This document describes the **target** architecture for ResumeIQ: the service boundaries we are building toward, the AI pipeline that is the product's core, the Azure infrastructure that hosts it, and a phased roadmap. It deliberately separates **what already exists** (the mock frontend on `parsing-feature`, and the Azure-backed reference implementation on `origin/Aasrith`) from **what we must build** to reach production.

---

## 1. Design goals

1. **Production-quality, cloud-deployable** — Azure-native, Terraform-defined, CI/CD-delivered. Nothing in this document requires a laptop.
2. **Clean separation of concerns** — frontend, backend/API, AI services, background workers, and infrastructure are distinct boundaries with explicit contracts, so the internship team can parallelize and the architecture can outgrow any one person.
3. **Deterministic-first, AI-augmented** — every score is computable without an LLM. AI refines and *explains*; it never replaces the auditable core. This keeps cost bounded and decisions defensible.
4. **Evidence-backed by construction** — every claim a recruiter sees traces to a quoted source in a resume. No score, verdict, or copilot answer is fabricated.
5. **Graceful degradation** — the product works with zero Azure credentials (heuristic parsers, keyword matching) and clearly labels which engine produced each result. Cloud is an upgrade, not a dependency.
6. **PII-safe** — resume data is personal data. Auth, encryption at rest, least-privilege, and auditability are core requirements, not features.
...
```
                         ...
                         └───────────────┬──────────────────────────────┘
                                         ...
                                         ▼
                          ┌────────────────────────────┐
                          │        Web app (BFF)       │  TanStack Start SSR
                          │  UI · auth sessions · proxy│  React + Server Fns
                          └───────────────┬────────────┘
                                          │ HTTPS (REST)
                                          ▼
                          ┌────────────────────────────┐
                          │       API service          │  Workflows: ingestion,
                          │  validation · orchestration│  screening, jobs, queries
                          └───┬──────────────┬─────────┘
                              │              │
              ┌───────────────▼────┐   ┌─────▼──────────────────┐
              │  Background worker │   │     AI engine          │  provider-agnostic
              │  (queue consumer)  │   │ parse · JD · match ·   │  Azure OpenAI + OSS
              │                    │   │ embed · agent          │  fallbacks
              └───────────────┬────┘   └─────┬──────────────────┘
                              │              │
                              ▼              ▼
   ┌─────────────────────────────────────────────────────────────┐
   │                       Azure data plane                      │
   │  Cosmos DB (records) · Blob Storage (files) · Key Vault     │
   │  (later) Cognitive Search (vector) · App Insights (telemetry)│
   └─────────────────────────────────────────────────────────────┘
                              ▲
                              │ control plane
   ┌─────────────────────────────────────────────────────────────┐
   │  Terraform (Azure resources) · GitHub Actions (CI/CD)       │
   └─────────────────────────────────────────────────────────────┘
```

**Request flow (screening example):**

1. Recruiter uploads resumes → web app streams files to **Blob Storage** and enqueues an **ingest job**.
...
3. Recruiter pastes a job description → API calls **AI engine** → structured `Requirement[]` returned, stored as a `JobRecord`.
4. Recruiter starts screening → API snapshots completed resumes, enqueues a **screening run**.
5. **Worker** runs the three-signal matcher (keyword → semantic embeddings → LLM best-first), persists `MatchRecord`s, publishes progress.
...
- **Web ↔ API:** web talks to API only over authenticated REST. Server functions must not reach Azure directly (today's `createServerFn` handlers call server modules directly — that's fine for the modular-monolith phase, but the *contract* is the API shape in `src/lib/api/*`, not the module internals).
- **API ↔ AI engine:** API passes domain objects (`ParsedResume`, `JobRecord`, `Requirement[]`) to the AI engine and gets structured, validated results back. The API never sees prompts, schemas, model names, or API keys. **This seam is what makes model-provider swaps (Azure ↔ OSS) safe.**
- **Worker ↔ AI engine:** the worker is the only caller of the expensive per-candidate analysis. Screening is never run on the API request thread.
...
- **Secrets:** only the AI engine and the web/worker bootstrap touch secrets, and only via **Key Vault with managed identity** — never environment files in production, never the client bundle.
...
| **RPC** | `createServerFn` endpoints (web↔server contract) | `lib/api/jobs.ts`, `lib/api/resumes.ts` |
...
| Embedding vectors | Cosmos DB (small, hot) → **Azure AI Search** (later, for candidate similarity search) | Vector index + hybrid keyword/vector retrieval |
...
**Migration path from today:** `.data/store.json` (reference) → Cosmos DB with a one-time script. Keep the store behind the `store.ts` interface so the swap is contained. **Do not ship the JSON-file store to production** — it is single-process, non-durable across restarts under concurrency, and unencrypted.
...
- `matches` container: partition by `jobId`, keyed by `resumeId` (the reference already models `jobId → resumeId → MatchRecord`).
...
The RPC layer on `Aasrith` already defines the right verbs — promote them to a versioned REST surface when the API splits out:

```
GET    /capabilities            → AzureCapabilities (what the UI can offer)
GET    /jobs/active             → JobSnapshot        (job + run + pool size + caps)
POST   /jobs/analyze            → analyze JD → JobRecord
PUT    /jobs/{id}/requirements  → save reviewed requirements (invalidates matches)
POST   /jobs/{id}/screening     → start screening run (async)
GET    /jobs/{id}/screening     → run progress
GET    /jobs/{id}/candidates    → ranked Candidate[]
GET    /resumes                 → ResumeRecord[] + counts + caps
POST   /resumes/upload          → multipart, batch ≤15 files
POST   /resumes/{id}/retry
POST   /resumes/retry-failed
POST   /resumes/cancel-queued
POST   /resumes/{id}/duplicate  → { skip | replace }
DELETE /resumes                 → clear all
```
...
```
Resume files ──► [1 Ingest] ──► [2 Extract] ──► [3 Parse] ──► ParsedResume
                                              (text)          (structured)

Job description ──► [4 JD analyze] ──► Requirement[]

ParsedResume[] + Requirement[] ──► [5 Match] ──► MatchRecord[]
                                          │
                                   keyword → semantic → AI
                                          │
                               [6 Rank & explain] ──► Candidate[] (UI)

Recruiter question ──► [7 Copilot agent] ──► evidence-backed answer
```

### 7.2 Stage-by-stage

**1. Ingest** (`server/pipeline.ts`)
- Batch uploads (≤15 files/request), SHA-256 content hash → duplicate detection by content, not filename.
- Size caps (`RESUME_MAX_FILE_BYTES`), typed stage machine `queued → uploading → extracting → ocr → parsing → complete|failed|duplicate|skipped`.
...
- `prebuilt-read` via Azure Document Intelligence for born-digital text and transparent OCR of scans; async submit → poll with backoff; legacy `/formrecognizer` fallback for old resources.
- Failure taxonomy preserved to the UI: *password-protected, empty scan, unsupported format, Azure rejected file* — each retryable with the actual reason shown.

**3. Parse** (`resume-parser.ts`)
- Azure OpenAI chat, JSON-schema-constrained → `ParsedResume`. System prompt enforces **never invent, never infer**; evidence quoted per skill.
- Coercion layer (`coerce`) sanitizes model output; `backfill` fills contact details from regex when the model skipped them.
- **Heuristic fallback** (regex/keyword parser) when chat is unavailable — labeled `"heuristic"` in the UI.

**4. JD analyze** (`jd-analyzer.ts`)
- Azure OpenAI → discrete `Requirement[]` with category, MUST/NICE, keyword aliases, `minYears`. Prompt rule: *must = true only when the JD states it as required/essential*.
...
**5. Match** (`matching.ts`) — **three-signal scoring, the heart of the product**

| Signal | What | Deterministic? | Cost |
|---|---|---|---|
| **Keyword** | Requirement keywords matched against the *relevant resume section* (out-of-section hits count half); tenure check for "N+ years" | ✅ yes | ~0 |
| **Semantic** | Cosine similarity between requirement and per-category resume embeddings (Azure embeddings, disk-cached) | ✅ yes (embeddings) | low, cached |
| **AI** | Per-candidate LLM: per-requirement `met/partial/missing` verdicts with quoted evidence, category scores, strengths/gaps/transferable, summary | ❌ | high — **best-first over top `SCREENING_AI_ANALYSIS_LIMIT` (default 50)** |
...
- Raw category scores stored server-side; recruiter `Weights` applied client-side → **instant re-ranking** without recomputation.
...
**7. Copilot agent** (Level 1 — currently a rule-based placeholder)
...
```
          ...
          │        ▲                 ▲                │
          │   ┌────┴───┐         ┌──┴────┐           │
          │  AzureOpenAI        OpenAICompat/OSS    │
          │  (primary)          (Ollama/vLLM/…)     │
          └──────────────────────────────────────────┘
```

- Both implementations share the OpenAI-compatible REST contract; Azure adds `api-key` headers + deployment URLs. The reference's `openai.ts` is now the `server/ai/` seam (Step B) — the app never imports Azure directly.
...
├── App: Web app (TanStack Start)         — Container App / App Service (Node)
├── App: API service                      — Container App / App Service (Node)
├── App: Worker (queue consumer)          — Container App (scale-to-zero off)
├── Data: Cosmos DB (NoSQL)               — records (resumes/jobs/matches/runs)
├── Data: Storage account                 — blob container `resumes` (private)
├── Data: Key Vault                       — OpenAI/DI keys, DB connection strings
├── AI:  Azure OpenAI (deployments: chat, embeddings)
├── AI:  Azure AI Document Intelligence
├── Obs: Log Analytics + Application Insights
...
│   └── prod/           ...
├── modules/
│   ├── web/            container app, DNS, WAF rule
│   ├── api/            container app, API gateway/config
│   ├── worker/         container app + queue wiring
│   ├── data/           cosmos db, storage, key vault
│   ├── ai/             openai + document intelligence (with purge-protection)
│   └── observability/  log analytics, app insights
└── main.tf             backend = Azure Storage (state locking)
```
...
### 8.3 CI/CD (GitHub Actions — target)

```
on: push / pull_request
├── CI: bun install → lint → typecheck → test (Vitest) → build
├── CD (on merge to main):
│   ├── terraform plan/apply (env: dev)
│   ├── docker build & push (ACR) for web/api/worker
│   ├── deploy (Container Apps)  → smoke test
│   └── promote staging → prod (manual approval gate)
└── scheduled: cost/health checks, model-deployment revalidation
```
...
| Client-visible | only `VITE_*` non-secrets; the reference's server-side `.env` loader keeps secrets off the bundle — preserve this invariant |
...
- **Distributed tracing** across web → API → worker → AI.
...
- Deterministic-first design means the LLM runs only on the top-N shortlist — cap with `SCREENING_AI_ANALYSIS_LIMIT` and monitor per-run tokens.
...
1. **AuthN/AuthZ:** recruiters authenticate (Azure Entra ID recommended); API enforces authorization per action; uploads restricted to authed users. **None of this exists today** — it is the first hardening gate before any real data.
2. **PII handling:** resumes are personal data. Private blob container, encryption at rest (Azure default), no PII in logs/metrics, retention/deletion policy (the UI already has "clear all" — make it auditable).
...
6. **Data egress:** OpenAI/DI calls carry resume content — document that this is a GDPR Art. 28 processor relationship and keep Azure region aligned with the data residency requirement.
...
|---|---|
| 500+ resume batch | Batch upload (≤15), async ingestion, concurrency pool, typed progress; scale worker on queue depth |
...
### Phase 0 — Reconcile & stabilize (foundation)
...
### Phase 1 — MVP hardening (ship a real, safe MVP)
...
- Adjustable scoring (weights) — already designed; surface per-signal impact.
...
- GitHub Actions CI/CD (lint → typecheck → test → build → terraform → deploy → smoke).
- Observability (App Insights, metrics, alerts), cost dashboards.

### Phase 4 — Scale & quality
- AI engine provider abstraction (Azure ↔ OpenAI/OSS), embeddings cache promotion.
...
2. **Deployment target:** Azure Container Apps (recommended — managed, worker-friendly) vs. App Service vs. Cloudflare Workers (`nodejs_compat`; cheaper but worker/queue story is weaker).
3. **Screening trigger:** auto-run on upload+JD vs. explicit "Start screening" (reference uses explicit — keep it; it makes cost visible).
...
*This document is a target design. Where it disagrees with the current code, the code is the source of truth for today and this document is the plan. Update this document when contracts or boundaries change — the schema for `ParsedResume`/`MatchRecord` in `src/lib/types.ts` is the contract all services share.*
